...
```json
{
  ...
  "observed_at": "RFC3339",
  "phase": "BACKEND_PREFLIGHT|RENDERER_READY|OUTPUTS_READY|WAYLAND_PROTOCOLS_READY|WAYLAND_PROTOCOL_ERROR|OUTPUT_ERROR|OUTPUT_REVALIDATION_REQUIRED|SESSION_PAUSED|RENDERER_REVALIDATION_REQUIRED|SESSION_RESUME_FAILED",
...
That state is not a public Wayland seat. Until the later input phase explicitly earns it, Prime does not call `new_seat`, does not register `delegate_seat!`, does not publish `wl_seat`, does not add input capabilities and does not route libinput events into Wayland clients.
