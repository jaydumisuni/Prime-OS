...
log "Rebuild Prime with canonical UKI seal"
"${PODMAN[@]}" build \
  --cap-add=all \
  --device /dev/fuse \
  --security-opt label=disable \
...
log "Prove final OCI-storage digest equals embedded UKI digest"
...
EMBEDDED_DIGEST="$("${PODMAN[@]}" run --rm --entrypoint /bin/bash localhost/prime-os:p1 -ceu '
...
[[ "$EMBEDDED_DIGEST" == "$CANONICAL_DIGEST" ]] || fail "UKI Composefs digest does not match canonical storage digest"
printf 'final digest:     %s\nembedded digest:  %s\n' "$FINAL_DIGEST" "$EMBEDDED_DIGEST"

log "Inspect final Prime image contract"
"${PODMAN[@]}" run --rm \
...
python3 -c 'import json,sys; d=json.load(open(sys.argv[1])); assert d["UnifiedKernel"] is True,d; assert d["Arch"]=="amd64",d; assert d["Bootloader"]=="systemd",d; o=d["OSInfo"]["OSRelease"]; assert o["ID"]=="fedora",d; assert o["VersionID"]=="44",d' "$RUN_DIR/prime-image-builder-inspect.json"
...
log "Inspect GPT, ESP and UKI placement"
cleanup_nbd
...
if [[ -n "$XBOOTLDR" ]]; then
  ...
  find "$MOUNT_XBOOTLDR" -maxdepth 5 -type f -name '*.efi' -print -quit | grep -q . || fail "UKI missing from XBOOTLDR"
else
  find "$MOUNT_ESP" -maxdepth 5 -type f -name '*.efi' -print -quit | grep -q . || fail "UKI missing from ESP"
fi
cleanup_nbd

log "Boot QCOW2 through OVMF/QEMU"
...
export REPORT SOURCE_REVISION CREATED_AT GENERATION_ID BASE_IMAGE BASE_DIGEST BUILDER_REF BUILDER_DIGEST BUILDER_VERSION CANONICAL_DIGEST FINAL_DIGEST EMBEDDED_DIGEST DISK DISK_SHA256 HOST_ID
python3 -c 'import json,os,pathlib; p={"schema":"prime.p1-local-proof.v1","ok":True,"source_revision":os.environ["SOURCE_REVISION"],"created_at":os.environ["CREATED_AT"],"generation_id":os.environ["GENERATION_ID"],"base_image":os.environ["BASE_IMAGE"],"base_image_digest":os.environ["BASE_DIGEST"],"image_builder":os.environ["BUILDER_REF"],"image_builder_digest":os.environ["BUILDER_DIGEST"],"image_builder_version":os.environ["BUILDER_VERSION"],"composefs_canonical_digest":os.environ["CANONICAL_DIGEST"],"composefs_final_digest":os.environ["FINAL_DIGEST"],"uki_embedded_digest":os.environ["EMBEDDED_DIGEST"],"qcow2_path":os.environ["DISK"],"qcow2_sha256":os.environ["DISK_SHA256"],"qemu_uefi":"OVMF", "prime_host_id":os.environ["HOST_ID"],"physical_kratos_boot_proven":False}; path=pathlib.Path(os.environ["REPORT"]); path.write_text(json.dumps(p,indent=2)+"\n",encoding="utf-8"); print(path.read_text())'
